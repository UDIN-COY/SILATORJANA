  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 border-b border-slate-100/60 pb-4">
        ...
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-800">Pemeriksaan Usulan</h2>
          <p className="text-slate-500 text-[15px] font-medium tracking-tight mt-1">{data.nama_kegiatan}</p>
        </div>
      </div>
      ...
      <div className="flex bg-slate-100/50 rounded-xl border border-slate-200/60 p-1.5 gap-1.5 overflow-x-auto shadow-sm">
        {tabs.map(tab => (
          <button
            ...
            className={`flex-1 px-4 py-2.5 text-[13px] font-bold uppercase tracking-wider rounded-lg transition-all whitespace-nowrap text-center flex items-center justify-center gap-2.5 ${
              activeTab === tab.id 
          ...
          >
            <tab.icon className={`size-4 ${activeTab === tab.id ? 'opacity-100' : 'opacity-70'}`} />
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {activeTab === 'info' && (
            <Card className="shadow-sm border-slate-200/60 bg-white overflow-hidden relative">
              ...
              <CardHeader className="border-b border-slate-100/60 bg-slate-50/30 py-4 pl-8">
              ...
              <CardContent className="p-8 space-y-6">
                ...
                {data.deskripsi && (
                  <div className="pt-6 border-t border-slate-100/80">
                    ...
                    <p className="text-[15px] text-slate-700 leading-relaxed bg-slate-50/80 p-4 rounded-xl border border-slate-100/60 shadow-sm">
                      {data.deskripsi}
                    </p>
                  </div>
                )}
              ...
              <CardContent className="p-8">
                {pengusulData ? (
                  ...
                  <div className="space-y-8">
                     <div className="text-amber-700 bg-amber-50/70 p-4 rounded-xl border border-amber-200/50 shadow-sm flex items-start gap-4">
                        <AlertCircle className="size-5 text-amber-500 shrink-0 mt-0.5" />
                        <p className="text-sm font-medium leading-relaxed">Profil pengguna sistem lengkap tidak tersedia karena belum didaftarkan di database Users. Menampilkan riwayat entri publik dari formulir kegiatan.</p>
                     </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          ...
          {activeTab === 'rab' && (
            <Card className="shadow-sm border-slate-200/60 bg-white overflow-hidden relative">
              ...
              <CardHeader className="border-b border-slate-100/60 bg-slate-50/30 py-5 pl-8 flex flex-row items-center justify-between">
                ...
                <div className="bg-emerald-50 border border-emerald-200/60 px-4 py-2 rounded-xl text-emerald-800 font-bold shadow-sm">
              ...
              <CardContent className="p-8">
                {ikuData.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {ikuData.map((iku, idx) => (
                      <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex flex-col justify-between hover:border-teal-300/50 transition-colors group">
                        <div className="mb-4">
                           <div className="w-8 h-8 rounded-full bg-teal-50 flex items-center justify-center mb-3">
                              <Target className="size-4 text-teal-600" />
                           </div>
                           <p className="font-bold text-[15px] text-slate-800 leading-snug group-hover:text-teal-800 transition-colors">{iku.nama_iku || iku.indikator || 'Entri IKU Tanpa Nama'}</p>
                           {iku.master_id && <p className="text-[11px] font-bold text-slate-400 mt-2 tracking-wider uppercase">Master Ref ID: <span className="text-slate-500">{iku.master_id}</span></p>}
                        </div>
                        {iku.target_persen != null && (
                          <div className="pt-4 border-t border-slate-100/80 flex items-end justify-between">
                            <p className="text-[11px] uppercase tracking-widest font-bold text-slate-400">Pencapaian Target</p>
                            <p className="text-2xl font-black text-teal-600">{iku.target_persen}%</p>
                          </div>
                        )}
                      </div>
                    ))}
        ...
        {/* Panel Aksi */}
        <div className="space-y-6">
           <Card className="shadow-lg shadow-slate-200/50 border-slate-200/60 bg-white sticky top-24 rounded-2xl overflow-hidden">
              <div className="h-1.5 w-full bg-gradient-to-r from-emerald-400 to-emerald-600"></div>
              <CardHeader className="bg-slate-50/30 border-b border-slate-100/60 py-5">
                 <CardTitle className="text-lg text-slate-800">Ruang Keputusan Akhir</CardTitle>
                 <CardDescription className="text-sm font-medium">Berikan penilaian putusan verifikasi level Anda pada dokumen ini.</CardDescription>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                 <div className="space-y-2.5">
                    <Label htmlFor="catatan" className="text-xs font-bold uppercase tracking-widest text-slate-500">Catatan Lampiran Opini</Label>
                    <textarea 
                      id="catatan"
                      value={catatan}
                      onChange={e => setCatatan(e.target.value)}
                      className="flex min-h-[120px] w-full rounded-xl border border-slate-200/80 bg-slate-50/50 px-4 py-3 text-[14px] leading-relaxed shadow-inner placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-emerald-500/10 focus-visible:border-emerald-500 focus-visible:bg-white transition-all resize-none"
                      placeholder="Tuliskan rekomendasi atau catatan alasan jika meminta agenda revisi (opsional untuk disetujui, Wajib apabila diminta revisi atau ditolak)..."
                    />
                 </div>
                 
                 <div className="flex flex-col gap-3 pt-2">
                    <Button disabled={isUpdating} onClick={() => updateStatus('verified')} className="w-full bg-[#047857] hover:bg-[#065F46] shadow-md shadow-emerald-700/20 text-white h-12 rounded-xl font-bold transition-all active:scale-95 text-[14.5px]">
                       {isUpdating ? <Loader2 className="size-4.5 mr-2 animate-spin" /> : <Check className="size-4.5 mr-2.5" />} Verifikasi & Setujui Dokumen
                    </Button>
                    <Button disabled={isUpdating} onClick={() => navigate(`/dashboard/verifikator/revisi/${id}`)} className="w-full bg-amber-500 hover:bg-amber-600 shadow-sm text-white h-12 rounded-xl font-bold transition-all active:scale-95 text-[14.5px]">
                       {isUpdating ? <Loader2 className="size-4.5 mr-2 animate-spin" /> : <AlertCircle className="size-4.5 mr-2.5" />} Kembalikan untuk Revisi
                    </Button>
                    <Button disabled={isUpdating} onClick={() => updateStatus('rejected')} variant="outline" className="w-full text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700 h-12 rounded-xl border-2 font-bold transition-all active:scale-95 text-[14.5px] bg-white">
                       {isUpdating ? <Loader2 className="size-4.5 mr-2 animate-spin" /> : <X className="size-4.5 mr-2.5" />} Tolak Permanen Usulan
                    </Button>
                 </div>
              </CardContent>
           </Card>
